/* Program Created By CHANDRESH GUPTA
    CLASS/SECTION: B.TECH 2nd YEAR COMPUTER ENGG./A2PE
    SERIAL NUMBER: 02
    FACULTY NUMBER: 16PEB006
    ENROLLMENT NUMBER: GJ6410 */
#include<stdio.h>
#include<stdlib.h>
...
void main()
{
    ...
    scanf("%d",&m);
    printf("\n\nEnter The No. Of Functions That You Want:");
    scanf("%d",&z);
    for(i=0;i<z;i++)
    {
            printf("\n\nEnter Data For Function %d",i+1);
            tabulation(m,i+1);
    }
}

void tabulation(int m,int j)
{
    int i,temp;
    gp=newgp=-1;
    for(i=0;i<m;i++)
    ...
    scanf("%d",&minterms);
    if(minterms>limit)
        printf("\nNo. Of Minterms Exceeds The Max. Value(i.e %d)",limit);
    else
    {
        p:
        if(minterms==0)
...
void expression(int n)   //converts binary into its expression form
{
    ...
    char y[]= {'a','b','c','d','e','f','g','h','i','j','k','l','m','n'};
    while(c!=m)
    {
        if(tab.a[n][c]!=-1)
        {
